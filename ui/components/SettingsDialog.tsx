...
function CodexSettingsPane() {
  ...
  const invalidateAuth = () =>
    queryClient.invalidateQueries({
      queryKey: getGetCodexAuthStatusQueryKey(),
    });
  ...
  return (
    ...
    >
      ...
      <div className="rounded-md border border-border bg-card p-3">
        ...
        {(actionError ||
          (auth?.login?.status === "failed" && auth.login.error)) && (
            ...
            {signedIn || loginStatus === "succeeded" ? (
              ...
              <>
                ...
                <span className="line-clamp-2">
                  {auth?.login?.error ?? t("ai.signInFailed")}
                </span>
              </>
            ) : (
              <>
        ...
        >
          <div className="divide-y divide-border overflow-hidden rounded-xl border border-border bg-card">
            {SHORTCUT_ITEMS.map((item) => {
              ...
              const hasConflict =
                currentVal && (conflictCounts.get(currentVal) || 0) > 1;
              const conflictingItem = hasConflict
                ? SHORTCUT_ITEMS.find(
                    (s) =>
                      s.key !== item.key &&
                      pendingShortcuts[s.key] === currentVal,
                  )
                : null;
